...
import RightSide from "../../components/clerks/RightSide";
import ClerkDocsInfo from "../../components/clerks/ClerkDocsInfo";
import CompletedDocs from "../../components/clerks/CompletedDocs";
...
function Clerkboard(props: Props) {
  ...
  const isAuthorized = decoded_token
    ? (decoded_token as any).user &&
      (decoded_token as any).user.roles.toLowerCase() === "clerk"
    : false;

  useEffect(() => {
    if (isAuthorized) navigate("/clerk");
    else navigate("/login");
      ...
      <div className="d-flex flex-grow-1">
        ...
        <div className="col-md-10 col-lg-10 pe-5">
          ...
          {page == "info" && (
            <ClerkDocsInfo
              setPage={setPage}
              curClerk={decoded_token ? (decoded_token as any).user.email : ""}
              searchKey={searchKey}
            />
          )}
          {page == "docs" && (
...
export default connect(mapStateToProps, { logout })(Clerkboard);
